...
class ToolHandler():
    def __init__(self, tool_name: str):
        print(f"Initializing ToolHandler for tool: {tool_name}")
        self.name = tool_name

    def get_account_descriptions(self) -> list[str]:
        print(f"Getting account descriptions for tool: {self.name}")
    ...
    def get_supported_emails_tool_text(self) -> str:
        print(f"Getting supported emails tool text for tool: {self.name}")
    ...
    def get_user_id_arg_schema(self) -> dict:
        print(f"Getting user ID arg schema for tool: {self.name}")
        return {
            "type": "string",
    ...
    def get_tool_description(self) -> Tool:
        print(f"Getting tool description for tool: {self.name}")
